            ...
            .mutation(async ({ input, ctx }) => {
                try {
                    ...
                    if (isSamePassword) {
                        throw new TRPCError({
                            code: "BAD_REQUEST",
                            message: "新しいパスワードが現在のパスワードと同じです。別のパスワードを設定してください",
                        })
                    }
            ...
            .mutation(async ({ input }) => {
                try {
                    ...
                    if (existingToken) {
                        throw new TRPCError({
                            code: "BAD_REQUEST",
                            message: "既にパスワード再設定用のメールをお送りしました。1時間後に再度お試しください",
                        })
                    }
                ...
                .mutation(async ({ input }) => {
                    try {
                        ...
                        if (!foundToken) {
                            throw new TRPCError({
                                code: "BAD_REQUEST",
                                message: "無効なトークンです。改めてパスワード再設定を行ってください",
                            })
                        }
                        ...
                        if (now > foundToken.expiry) {
                            throw new TRPCError({
                                code: "BAD_REQUEST",
                                message: "トークンの有効期限が切れています。改めてパスワード再設定を行ってください",
                            })
                        }
